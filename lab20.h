...
Unit::Unit(string t,string n){ 
    ...
    guard_on = false;
    dodge_on = false; 
    equipment = NULL; // เริ่มต้นโดยยังไม่มีอาวุธ
}
...
// --- ฟังก์ชันการสวมใส่อาวุธ ---
void Unit::equip(Equipment *new_eq){
    // 1. ถอดอุปกรณ์เดิมออก (ถ้ามี)
    if(equipment != NULL){
    ...
    // 2. เปลี่ยนอุปกรณ์ใหม่
    equipment = new_eq;
    
    // 3. บวกสถานะจากอุปกรณ์ใหม่
    vector<int> new_stat = equipment->getStat();
    hpmax += new_stat[0];
    atk += new_stat[1];
    def += new_stat[2];
    
    // 4. ตรวจสอบเลือด หากมากเกิน hpmax ใหม่ ให้ปรับลดลงมา
    if(hp > hpmax){
        hp = hpmax;
    }
}
